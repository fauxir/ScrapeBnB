...
function standardizeSearch(results) {
  ...
  for (const result of results) {
    ...
    const pr = getNestedValue(result, "pricingQuote.structuredStayDisplayPrice", {});
    ...
    if (splited.length === 2) {
      ...
      data.rating.reviewCount = reviewCountMatch ? parseInt(reviewCountMatch[0]) : 0;
    }

    let priceToUse = getNestedValue(pr, "primaryLine.originalPrice", "") || getNestedValue(pr, "primaryLine.price", "");
    if (priceToUse) {
    ...
    const discountedPrice = getNestedValue(pr, "primaryLine.discountedPrice", "");
    if (discountedPrice) {
      const [amount] = parsePriceSymbol(discountedPrice);
      data.price.unit.discount = amount;
    }

    const secondaryLineSplited = getNestedValue(pr, "secondaryLine.price", "").split(" ");
    ...
    for (const priceDetail of getNestedValue(pr, "explanationData.priceDetails", [])) {
      ...
      for (const item of priceDetail.items) {
        ...
        data.price.break_down.push({ description: item.description, amount, currency_symbol: currency });
...
function standardizeDetails(meta) {
  ...
  const sd = getNestedValue(meta, "data.presentation.stayProductDetailPage.sections.sections", []);

  for (const section of getNestedValue(sd, "sectionConfiguration.root.sections", [])) {
    const typeName = getNestedValue(section, "sectionData.__typename", "");
    if (typeName === "PdpHostOverviewDefaultSection") {
      data.host = {
        id: getNestedValue(section, "sectionData.hostAvatar.loggingEventData.eventData.userId", ""),
  ...
  for (const section of getNestedValue(meta, "data.presentation.stayProductDetailPage.sections.sections", [])) {
    ...
    switch (typeName) {
      case "HostProfileSection":
        ...
        for (const cohost of getNestedValue(section, "section.additionalHosts", [])) {
          data.co_hosts.push({ id: cohost.id || "", name: cohost.name || "" });
        }
        break;
      case "PhotoTourModalSection":
        for (const mediaItem of getNestedValue(section, "section.mediaItems", [])) {
          data.images.push({
      ...
      case "PoliciesSection":
        for (const houseRulesSection of getNestedValue(section, "section.houseRulesSections", [])) {
          const houseRule = {
          ...
          for (const item of houseRulesSection.items || []) {
            ...
            houseRule.values.push({ title: item.title || "", icon: item.icon || "" });
          }
          data.house_rules.general.push(houseRule);
        }
        break;
      case "LocationSection":
        for (const locationDetail of getNestedValue(section, "section.seeAllLocationDetails", [])) {
      ...
      case "PdpHighlightsSection":
        for (const highlightingData of getNestedValue(section, "section.highlights", [])) {
      ...
      case "AmenitiesSection":
        for (const amenityGroupRaw of getNestedValue(section, "section.seeAllAmenitiesGroups", [])) {
          const amenityGroup = {
